...
because shapes in their definition describe the properties (RDF predicate) of the entities whereas the type index only describes the classes IRIs.
It would be possible to dereference the class IRIs to get information about the properties however it is not the current practice \cite{Taelman2023}.
A comparison of the RDF data shapes and RDF class approach is delegated to future works.
It has to be noted that in the literature \cite{demeester_swj_2021} there are comparisons 
between the two approaches in the context of data validation but it is left to be determined if their frame of comparison is compatible with our problem.
