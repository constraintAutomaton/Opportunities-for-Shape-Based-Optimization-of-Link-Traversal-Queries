...
Additionally, an SI has an associated domain (set of URLs) \rt{Is it really a set? Or some kind of pattern?}
...
In a SI when a shape is in relation \rt{"in relation" -> "matches"?} to a set of RDF resources then the shape \emph{must} validate those resources.
Furthermore, every set of triples respecting the shape in the domain \emph{must} be located inside one resource of the set.

Our approach consists of determining before the traversal of a whole domain \sepfootnote{domain} the location of the useful resources for the query execution. \rt{This sentence needs rephrasing.}
This data discovery operation results in the pruning of links leading to resources not contributing to query results.
