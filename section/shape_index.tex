...
This case is similar to the usage of the type index but with a larger direct reach because the SI directly describes the properties of data whereas
the type index only describes the classes. 
Since a class is an IRI it is possible to find its properties (if they are defined) by performing another dereferencing operation.
However, it has to be noted that the current usage of the type index only match the class with the query and do not consider the properties \cite{Taelman2023}.
