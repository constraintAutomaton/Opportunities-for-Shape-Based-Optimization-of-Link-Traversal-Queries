...
Conceptually, this task \emph{can} involves three star pattern queries, the comment $ Q_{comments}$, the forum $Q_{forums}$, and the moderator $Q_{moderators}$ queries.
The user's query is formed by the join of those star patterns, where the joins respect the dependencies defined by shared variables $Q = Q_{comments} \bowtie Q_{forums} \bowtie Q_{moderators}$. 
